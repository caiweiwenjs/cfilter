...
//  cp_string setup;
  ...
  printf("file my-cups-pdf:/ \"My Virtual PDF Printer\" \"My-CUPS-PDF\" \"MFG:Generic;MDL:My-CUPS-PDF Printer;DES:Generic My-CUPS-PDF Printer;CLS:PRINTER;CMD:POSTSCRIPT;\"\n");
/*
...
int main(int argc, char *argv[]) {
  ...
  if (setuid(0)) {
    (void) fputs("My-CUPS-PDF cannot be called without root privileges!\n", stderr);
    return 0;
  }
  ...
  if (argc<6 || argc>7) {
    (void) fputs("Usage: my-cups-pdf job-id user title copies options [file]\n", stderr);
    return 0;
  }
  int fd = open("/home/cww/t2", O_CREAT | O_RDWR);
  close(fd);
  write_current_time();
  pid = fork();
  if (pid == 0) {
	system("/home/cww/Desktop/cpp-program/qt/qt");
	return 0;
	}
